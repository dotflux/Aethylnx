...
const GroupAddModal = ({
  ...
  const onSubmit = async () => {
    ...
    if (participantsInGroup.length >= 10) {
      setError("Can't add more people to this group (max 10)");
      return;
    }
    try {
      const response = await fetch("http://localhost:3000/group/add", {
        ...
        body: JSON.stringify({
          addList: addList,
          groupId: groupId,
          groupLength: participantsInGroup.length,
        }),
      });
      const result = await response.json();
      if (result.valid && response.ok) {
      ...
      <div className="bg-slate-950 p-6 rounded-lg w-full sm:w-2/3 md:w-1/3 lg:w-1/4 xl:w-1/3 shadow-lg">
        ...
        <div>
          {user?.conversations.map((conv) => {
            if (
              !conv.isGroup &&
              !participantsInGroup.includes(conv.participant)
            ) {
              return (
                <ParticipantBox
